...
Renderer::Renderer(const std::size_t screen_width,
  ...
  sdl_obstacles_surface = SDL_LoadBMP("../assets/bomb.bmp");
  sdl_obstacles_texture = SDL_CreateTextureFromSurface(sdl_renderer, sdl_obstacles_surface);  
}
...
void Renderer::Render(Snake const snake, SDL_Point const &food, const std::shared_ptr<Obstacles> obstacles) {
  SDL_Rect block;
...
void Renderer::placeObstacles(const std::shared_ptr<Obstacles> obstacles) const{
  for (const Coordinate &coordinate : obstacles->GetCoordinates()){
    ...
    SDL_RenderCopy(sdl_renderer, sdl_obstacles_texture, NULL, &obstacleRect);
  }
}